...
ns <- nrow(mHSPC_OS_doublet_ITT_sensitivity)
t  <- array(c(mHSPC_OS_doublet_ITT_sensitivity$t1, mHSPC_OS_doublet_ITT_sensitivity$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_OS_doublet_ITT_sensitivity$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_OS_doublet_ITT_sensitivity$se), dim = c(ns, 2))

study_names <- gsub("#", "", mHSPC_OS_doublet_ITT_sensitivity$`#ID`)
...
cross_hr_os_doublet_sensitivity_fe <- cross_effect(bugs_object = bugs_object_fe_mams, t_names = c("ADT", "DAR+ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "ABI+ADT", "APA+ADT", "SNA+ADT", "ABI+DOC+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_hr_os_doublet_sensitivity_fe, file = "cross_hr_os_doublet_sensitivity_fe.csv")
