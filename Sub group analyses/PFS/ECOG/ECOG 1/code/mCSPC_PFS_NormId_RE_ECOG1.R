...
ns <- nrow(mCSPC_PFS_data_ECOG_1)
t  <- array(c(mCSPC_PFS_data_ECOG_1$t1, mCSPC_PFS_data_ECOG_1$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_PFS_data_ECOG_1$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_PFS_data_ECOG_1$se), dim = c(ns, 2))
...
cross_hr_doublet_re_pfs_ECOG1 <- cross_effect(bugs_object = bugs_object_re, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "APA+ADT", "ABI+DOC+ADT", "ABI+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_hr_doublet_re_pfs_ECOG1, file = "cross_hr_doublet_re_pfs_ECOG1")
