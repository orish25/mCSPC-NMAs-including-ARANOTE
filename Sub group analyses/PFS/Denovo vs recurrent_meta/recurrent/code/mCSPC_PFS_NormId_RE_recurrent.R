...
ns <- nrow(mCSPC_PFS_data_recurrent)
t  <- array(c(mCSPC_PFS_data_recurrent$t1, mCSPC_PFS_data_recurrent$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_PFS_data_recurrent$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_PFS_data_recurrent$se), dim = c(ns, 2))
...
cross_hr_doublet_re_pfs_recurrent <- cross_effect(bugs_object = bugs_object_re, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ABI+ADT", "APA+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_hr_doublet_re_pfs_recurrent, file = "cross_hr_doublet_re_pfs_recurrent")

calculate_sucra(bugs_object = bugs_object_re, bugs_data = bugs_data, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ABI+ADT", "APA+ADT"))


# Define your treatment names
treatment_names <- c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ABI+ADT", "APA+ADT")
