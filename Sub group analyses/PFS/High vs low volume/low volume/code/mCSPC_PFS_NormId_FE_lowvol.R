...
ns <- nrow(mCSPC_PFS_data_low_volume)
t  <- array(c(mCSPC_PFS_data_low_volume$t1, mCSPC_PFS_data_low_volume$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_PFS_data_low_volume$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_PFS_data_low_volume$se), dim = c(ns, 2))
...
cross_doublet_fe_pfs_lowvol <- cross_effect(bugs_object = bugs_object_fe, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "APA+ADT", "ABI+DOC+ADT", "ABI+ADT", "SNA+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_doublet_fe_pfs_lowvol, file = "cross_doublet_fe_pfs_lowvol")

calculate_sucra(bugs_object = bugs_object_fe, bugs_data = bugs_data, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "APA+ADT", "ABI+DOC+ADT", "ABI+ADT", "SNA+ADT"))
