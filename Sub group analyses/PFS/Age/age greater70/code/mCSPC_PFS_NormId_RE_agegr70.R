...
ns <- nrow(mHSPC_PFS_data_age_greater70)
t  <- array(c(mHSPC_PFS_data_age_greater70$t1, mHSPC_PFS_data_age_greater70$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mHSPC_PFS_data_age_greater70$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mHSPC_PFS_data_age_greater70$se), dim = c(ns, 2))
...
cross_hr_doublet_re_pfs_agegr70 <- cross_effect(bugs_object = bugs_object_re, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ABI+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_hr_doublet_re_pfs_agegr70, file = "cross_hr_doublet_re_pfs_agegr70")

calculate_sucra(bugs_object = bugs_object_re, bugs_data = bugs_data, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ABI+ADT"))


# Define your treatment names
treatment_names <- c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ABI+ADT")
