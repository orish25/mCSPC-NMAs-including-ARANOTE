...
#Load the data
mCSPC_PFS_class_effects_together <- read_excel("mCSPC PFS class effects together.xlsx") 
...
ns <- nrow(mCSPC_PFS_class_effects_together)
t  <- array(c(mCSPC_PFS_class_effects_together$t1, mCSPC_PFS_class_effects_together$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_PFS_class_effects_together$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_PFS_class_effects_together$se), dim = c(ns, 2))

study_names <- gsub("#", "", mCSPC_PFS_class_effects_together$`#ID`)
...
cross_meandiff_fe_pfs_all_merged <- cross_effect(bugs_object = bugs_object_fe_mams, t_names = c("ADT", "DAR+ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "ABI+ADT", "APA+ADT", "SNA+ADT", "ABI+DOC+ADT", "ENZ+DOC+ADT", "SNA+DOC+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_pfs_all_merged, file = "cross_meandiff_fe_pfs_all_merged.csv")
...
write.csv(samples_flattened, file = "MCMC_samples.csv", row.names = FALSE)
...
# Save the samples to a CSV file
write.csv(samples_flattened_d, file = "MCMC_samples_d.csv", row.names = FALSE)
