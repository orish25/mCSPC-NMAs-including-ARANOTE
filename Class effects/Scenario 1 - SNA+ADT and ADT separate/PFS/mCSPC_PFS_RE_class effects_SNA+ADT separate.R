...
cross_meandiff_re_pfs_separate <- cross_effect(bugs_object = bugs_object_re_mams, t_names = c("ADT", "DAR+ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "ABI+ADT", "APA+ADT", "SNA+ADT", "ABI+DOC+ADT", "ENZ+DOC+ADT", "SNA+DOC+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_re_pfs_separate, file = "cross_meandiff_re_pfs_separate.csv")
...
write.csv(samples_flattened, file = "MCMC_samples_scenario2_PFS_RE.csv", row.names = FALSE)
...
# Save the samples to a CSV file
write.csv(samples_flattened_d, file = "MCMC_samples_scenario2_PFS_RE_d.csv", row.names = FALSE)
