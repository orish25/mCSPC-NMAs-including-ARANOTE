...
#Load the data
mCSPC_PFS_class_effects_separate <- read_excel("mCSPC PFS class effects separate.xlsx")
...
ns <- nrow(mCSPC_PFS_class_effects_separate)
t  <- array(c(mCSPC_PFS_class_effects_separate$t1, mCSPC_PFS_class_effects_separate$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_PFS_class_effects_separate$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_PFS_class_effects_separate$se), dim = c(ns, 2))

study_names <- gsub("#", "", mCSPC_PFS_class_effects_separate$`#ID`)
...
bugs_object_fe <- bugs(data = bugs_data, inits = bugs_inits,
                     parameters.to.save = c("d", "totresdev", "doublet_mean", "triplet_mean", "doublet_sd", "triplet_sd", "rk", "best", "prob"),
...
cross_meandiff_fe_pfs_separate <- cross_effect(bugs_object = bugs_object_fe_mams, t_names = c("ADT", "DAR+ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "ABI+ADT", "APA+ADT", "SNA+ADT", "ABI+DOC+ADT", "ENZ+DOC+ADT", "SNA+DOC+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_meandiff_fe_pfs_separate, file = "cross_meandiff_fe_pfs_separate.csv")
...
write.csv(samples_flattened, file = "MCMC_samples_scenario2_PFS.csv", row.names = FALSE)
...
# Save the samples to a CSV file
write.csv(samples_flattened_d, file = "MCMC_samples_scenario2_PFS_d.csv", row.names = FALSE)
