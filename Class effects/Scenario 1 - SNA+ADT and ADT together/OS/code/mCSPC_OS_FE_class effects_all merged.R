...
model_mams_adjust_fe <- function()
{
  ...
  d[1] <- 0 # ADT remains reference but in the same class with SNA+ADT
...
ns <- nrow(mCSPC_OS_doublet_ITT_class_effects_all_merged)
t  <- array(c(mCSPC_OS_doublet_ITT_class_effects_all_merged$t1, mCSPC_OS_doublet_ITT_class_effects_all_merged$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_OS_doublet_ITT_class_effects_all_merged$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_OS_doublet_ITT_class_effects_all_merged$se), dim = c(ns, 2))

study_names <- gsub("#", "", mCSPC_OS_doublet_ITT_class_effects_all_merged$`#ID`)
...
bugs_object_fe <- bugs(data = bugs_data, inits = bugs_inits,
                     parameters.to.save = c("d", "doublet_mean", "triplet_mean", "doc_mean", "adt_mean", "totresdev", "rk", "best", "prob"), 
