...
ns <- nrow(mCSPC_PFS_doublet_ITT_all_doublets_merged)
t  <- array(c(mCSPC_PFS_doublet_ITT_all_doublets_merged$t1, mCSPC_PFS_doublet_ITT_all_doublets_merged$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_PFS_doublet_ITT_all_doublets_merged$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_PFS_doublet_ITT_all_doublets_merged$se), dim = c(ns, 2))

study_names <- gsub("#", "", mCSPC_PFS_doublet_ITT_all_doublets_merged$`#ID`)
...
bugs_object_fe <- bugs(data = bugs_data, inits = bugs_inits,
                     parameters.to.save = c("d", "totresdev", "doublet_mean", "triplet_mean", "doc_mean", "adt_mean", "rk", "best", "prob"),
