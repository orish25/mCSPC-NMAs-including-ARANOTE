...
mCSPC_OS_doublet_ITT_inconsistency <- transform(mCSPC_OS_doublet_ITT_inconsistency, y = ifelse(t2 < t1, abs(y), y))
mCSPC_OS_doublet_ITT_inconsistency <- transform(mCSPC_OS_doublet_ITT_inconsistency, y = ifelse(X.ID == "#STAMPEDE-4", -(y), y))

temp <- mCSPC_OS_doublet_ITT_inconsistency$t1
mCSPC_OS_doublet_ITT_inconsistency <- transform(mCSPC_OS_doublet_ITT_inconsistency, t1 = ifelse(t1 > t2, t2, t1))
mCSPC_OS_doublet_ITT_inconsistency <- transform(mCSPC_OS_doublet_ITT_inconsistency, t2 = ifelse(t2 == t1, temp, t2))
...
ns <- nrow(mCSPC_OS_doublet_ITT_inconsistency)
t  <- array(c(mCSPC_OS_doublet_ITT_inconsistency$t1, mCSPC_OS_doublet_ITT_inconsistency$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_OS_doublet_ITT_inconsistency$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_OS_doublet_ITT_inconsistency$se), dim = c(ns, 2))

study_names <- gsub("#", "", mCSPC_OS_doublet_ITT_inconsistency$X.ID)
...
inits3_ume <- list(
  d = t(structure(.Data = c(
    ...
    NA,            NA,0.5,0.5,0.5,0.5,
    NA,            NA,            NA,            NA,            NA,
    NA,0.5,0.5,0.5,            NA,            NA,
    NA,            NA,            NA,            NA,            NA,
    0.5,0.5,            NA,            NA,            NA,            NA,
    NA,            NA,            NA,            NA, 
    0.5,            NA,            NA,            NA,            NA,
...
direct_evidence <- as.data.frame(mCSPC_OS_doublet_ITT_inconsistency[(mCSPC_OS_doublet_ITT_inconsistency$t1 == 2 & mCSPC_OS_doublet_ITT_inconsistency$t2 == 4),])
...
indirect_evidence <- as.data.frame(mCSPC_OS_doublet_ITT_inconsistency[(mCSPC_OS_doublet_ITT_inconsistency$t2 == 5),])
indirect_evidence$t2 <- 3
indirect_evidence$t1 <- ifelse(indirect_evidence$t1 == 2, 1, 2)
#Treatments 2, 4 and 5 relabeled 1, 2 and 3
...
#p_value_re <- sum(diff_re > 0)/length(diff_re)
