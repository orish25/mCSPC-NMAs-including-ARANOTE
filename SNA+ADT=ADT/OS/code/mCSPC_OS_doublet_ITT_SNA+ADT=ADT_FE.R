...
ns <- nrow(mCSPC_OS_doublet_ITT_SNA_ADT_ADT)
t  <- array(c(mCSPC_OS_doublet_ITT_SNA_ADT_ADT$t1, mCSPC_OS_doublet_ITT_SNA_ADT_ADT$t2), dim = c(ns, 2)) 
nt <- max(t) 
y  <- array(c(rep(0, ns), mCSPC_OS_doublet_ITT_SNA_ADT_ADT$y), dim = c(ns, 2))
se <- array(c(rep(0, ns), mCSPC_OS_doublet_ITT_SNA_ADT_ADT$se), dim = c(ns, 2))

study_names <- gsub("#", "", mCSPC_OS_doublet_ITT_SNA_ADT_ADT$`#ID`)
...
cross_hr_os_doublet_ADT_only_fe <- cross_effect(bugs_object = bugs_object_fe_mams, t_names = c("DAR+ADT", "ADT", "DAR+DOC+ADT", "DOC+ADT", "ENZ+ADT", "ABI+ADT", "APA+ADT", "ENZ+DOC+ADT", "ABI+DOC+ADT"), med = TRUE, exp = TRUE)
write.csv(x = cross_hr_os_doublet_ADT_only_fe, file = "cross_hr_os_doublet_ADT_only_fe.csv")
